...
import { Button, Error, File as FileInput, Input, Select, TextArea } from '../../components/ui';
...
interface FormValues {
 ...
 // 폼검증에 사용할 스키마
    ...
    .test(
        'end-after-start','종료 일시는 시작 일시 이후여야 합니다.',
         (value: unknown) => {
...
const TeamWrite: React.FC = () => {
    ...
    //미리보기
    const [preview, setPreview] = useState<string | ArrayBuffer | null>(dafaultimg);
    //파일 저장
    ...
    return (
        <div className={styles.container}>
            <form className={styles.write_wrap} onSubmit={handleSubmit(onValid, onInvalid)}>
                {/* 팀 이름 */}
                <div className={styles.write_box}>
                    <label className={styles.label}>제목 <span className='required'>*</span></label>
                    <Input {...register('teamName')} />
                    <Error isError={Boolean(errors.teamName)}>{errors.teamName && errors.teamName.message?.toString()}</Error>
                </div>

                {/* 내용 */}
                <div className={styles.write_box}>
                    <label className={styles.label}>내용 <span className='required'>*</span></label>
                    <TextArea {...register('conts')} />
                    <Error isError={Boolean(errors.conts)}>{errors.conts && errors.conts.message?.toString()}</Error>
                </div>
                
                {/* 주소 */}
                <div className={styles.write_box}>
                    <label className={styles.label}>주소 <span className='required'>*</span></label>
                    <div className={styles.write_box}>
                        <Input {...register('addr')} type='text' readOnly />
                        <div className={styles.write_addr_detail_box}>
                            <Input {...register('detailAddr')} type='text' ref={(el: HTMLInputElement | null) => {
                                register('detailAddr').ref(el);
                                detailAddrRef.current = el;
                                }}/>
                            <Button type='button'  onClick={onOpenDaumAddrAPI} >검색</Button>
                        </div>
                    </div>
                </div>

                {/* 카테고리 코드 */}
                <label className={styles.label}>카테고리 <span className='required'>*</span></label>
                ...
                <Error isError={Boolean(errors.categoryCd)}>{errors.categoryCd && errors.categoryCd.message?.toString()}</Error>
                {/* 시작일시 */}
                <div className={styles.write_box}>
                    <label className={styles.label}>시작일시 <span className='required'>*</span></label>
                    <div className={styles.write_box}>
                        ...
                        <Error isError={Boolean(errors.startDate)}>{errors.startDate?.message}</Error>
                        <Error isError={Boolean(errors.startTime)}>{errors.startTime?.message}</Error>
                    </div>
                </div>

                {/* 종료일시 */}
                <div className={styles.write_box}>
                    <label className={styles.label}>종료일시 <span className='required'>*</span></label>
                    <div className={styles.write_box}>
                        <Input {...register('endDate')} type='date' min={todayDate}/>
                        <Input {...register('endTime')} type='time'
                        min={watchEndDate === todayDate ? nowTime : '00:00'}/>
                        <Error isError={Boolean(errors.endDate)}>{errors.endDate?.message}</Error>
                        <Error isError={Boolean(errors.endTime)}>{errors.endTime?.message}</Error>
                    </div>
                </div>

                {/* 최대 인원 */}
                <div className={styles.write_box}>
                    <label className={styles.label}>최대 인원 <span className='required'>*</span></label>
                    <div className={styles.write_box}>
                    <input {...register('maxMemberCnt',{valueAsNumber: true})} type='number' min={1}/>    
                    </div>                
                </div>
                ...
                <div className={styles.write_box}>
                    ...
                    {preview &&
                        <img src={preview as string} alt="미리보기" style={{ width: '4.571rem'}} />}
                </div>               
