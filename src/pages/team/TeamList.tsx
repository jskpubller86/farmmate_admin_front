...
const TeamList: React.FC = () => {
  ...
  const [page, setPage] = useState(1); // 현재까지 불러온 페이지 번호
  ...
  const fetchTeams = async (pageNum: number) => {
    try {
      ...
      const res = await api.get("/team/readTeamList", {
        page: pageNum,
        size: PAGE_SIZE,
      });
  ...
  //컴포넌트 마운트(컴포넌트 최초 생성해 화면에 삽입하는 과정)시 1페이지를 처음 불러오기
  useEffect(() => {
    fetchTeams(1);
  }, []);
  ...
  return (
    <div className={styles.full_container}>
      ...
      <div className={styles.team_write_btn}>
        <Link to={"/team_write"}>+</Link> */}
      </div>
    </div>
  );
};
