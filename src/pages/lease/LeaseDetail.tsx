...
const LeaseDetail: React.FC = () => {
  ...
  const [leaseData, setLeaseData] = useState<LeaseDetailData | null>(null);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: leaseData?.title || '토지 임대',
        text: leaseData?.description || '',
  ...
  return (
    <div className={layout.container_full}>
      <div className={styles.lease_detail_container}>
        {/* 뒤로가기 버튼 */}
        ...
        {/* 제목 */}
        <h1 className={styles.lease_detail_title}>{leaseData.title}</h1>
        ...
        <div className={styles.action_buttons}>
          <Button
            onClick={handleLike}
            className={`${styles.action_button} ${isLiked ? styles.wish_button : ''}`}
            color="secondary"
          >
            <LikeIt isLiked={isLiked} onClick={handleLike} />
            찜하기
          </Button>
          <Button
            onClick={handleShare}
