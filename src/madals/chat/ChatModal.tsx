...
const Chat: React.FC<Props> = ({ modalId, channelId }) => {
  ...
  const appendLog = (logData: LogProps[] | LogProps) => {
    if (Array.isArray(logData)) {
      ...
      setLogs((prev) =>
        filteredLogs([...deleteNewTag(prev), { ...logData, isNewLog: false }])
      );
  ...
  const printDate = (log: LogProps) => {
    return `${log.creDatetime.getFullYear()}년 ${
      log.creDatetime.getMonth() + 1
    }월 ${log.creDatetime.getDate()}일`;
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header_box}>채팅방</div>

      {/*  메세지 영역 */}
      <div className={styles.message_box}>
        <div
          className={styles.message_group}
          ref={mssgBoxRef}
        ...
        >
          ...
          {logs.map((log, idx, list) => {
            return (
              <>
                {idx != 0 &&
                  !isSameDate(log.creDatetime, list[idx - 1].creDatetime) && (
                    <div key={`date${idx}`} className={styles.message_item}>
                      <div className={styles.date}>{printDate(log)}</div>
                    </div>
                  )}
                <div
                  key={idx}
                  data-new-log={log.isNewLog ? 1 : 0}
                  className={styles.message_item}
                >
                  <div
                    className={
                      log.isMine ? styles.my_unit_box : styles.another_unit_box
                    }
                  >
                    ...
                    <div className={styles.unit_messge_box}>
                      ...
                      <div className={styles.unit_message_wrap}>
                        <p className={styles.unit_message}>
                          {log.message}{" "}
                          <span className={styles.time}>{printTime(log)}</span>
                        </p>
                      </div>
                    </div>
                  </div>
